...
async function setupSupabase() {
  ...
  try {
    ...
    const createTablesSQL = `
      ...
      CREATE INDEX IF NOT EXISTS idx_user_profiles_pump_id ON user_profiles(petrol_pump_id);
      CREATE INDEX IF NOT EXISTS idx_user_profiles_role ON user_profiles(role);
      
      CREATE INDEX IF NOT EXISTS idx_dispensers_pump_id ON dispensers(petrol_pump_id);
      CREATE INDEX IF NOT EXISTS idx_dispensers_code ON dispensers(dispenser_code);
      CREATE INDEX IF NOT EXISTS idx_dispensers_fuel_type ON dispensers(fuel_type);
      
      CREATE INDEX IF NOT EXISTS idx_charging_points_pump_id ON charging_points(petrol_pump_id);
      CREATE INDEX IF NOT EXISTS idx_charging_points_code ON charging_points(charging_point_code);
      CREATE INDEX IF NOT EXISTS idx_charging_points_status ON charging_points(status);
      
      CREATE INDEX IF NOT EXISTS idx_daily_rates_pump_fuel ON daily_rates(petrol_pump_id, fuel_type);
      CREATE INDEX IF NOT EXISTS idx_daily_rates_date ON daily_rates(effective_date);
      CREATE INDEX IF NOT EXISTS idx_daily_rates_active ON daily_rates(is_active);
      
      CREATE INDEX IF NOT EXISTS idx_charging_sessions_point_id ON charging_sessions(charging_point_id);
      CREATE INDEX IF NOT EXISTS idx_charging_sessions_operator_id ON charging_sessions(operator_id);
      ...
      CREATE INDEX IF NOT EXISTS idx_shifts_dispenser_id ON shifts(dispenser_id);
      CREATE INDEX IF NOT EXISTS idx_shifts_operator_id ON shifts(operator_id);
      ...
      CREATE TRIGGER update_petrol_pumps_updated_at BEFORE UPDATE ON petrol_pumps FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
      ...
      CREATE TRIGGER update_charging_points_updated_at BEFORE UPDATE ON charging_points FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
      ...
      CREATE TRIGGER calculate_shifts_fuel_sold BEFORE INSERT OR UPDATE ON shifts FOR EACH ROW EXECUTE FUNCTION calculate_fuel_sold();
    `;
    ...
    if (error) {
      ...
      for (const table of tables) {
        const { error: tableError } = await supabaseAdmin.rpc('exec_sql', { sql: table.sql });
        if (tableError) {
    ...
    const { error: rlsError } = await supabaseAdmin.rpc('exec_sql', { sql: rlsPolicies });
    if (rlsError) {
      console.log('⚠️  RLS policies setup had issues (this is normal for new projects):', rlsError.message);
    } else {
    ...
    console.log('3. Run the SQL commands from scripts/manual-setup.sql');
  }
}
